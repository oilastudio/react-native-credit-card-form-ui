...
import { View, Text, StyleSheet, Image, Animated, TouchableOpacity, TouchableWithoutFeedback, Keyboard, PixelRatio, Platform, } from 'react-native';
...
const validationSchema = yup.object().shape({
    holder: yup
        ...
        const { isPotentiallyValid, isValid } = cardValidator.cardholderName(holderName);
        return runtime ? isPotentiallyValid : isValid;
    }),
    number: yup
        .string()
        .defined()
        .test('is-valid-card-number', 'Card number is invalid', function (cardNumber) {
        const { runtime = false } = this.options.context;
    ...
    expiration: yup
        .string()
        .defined()
        .test('is-valid-expiration', 'Card expiration is invalid', function (expiration) {
        const { runtime = false } = this.options.context;
        const { isPotentiallyValid, isValid } = cardValidator.expirationDate(expiration);
        return runtime ? isPotentiallyValid : isValid;
    }),
    cvv: yup
        .string()
...
const CreditCard = React.forwardRef(({ placeholders, labels, background, textColor, errorTextColor, placeholderTextColor, initialValues, expirationDateFormat, onValidStateChange, }, ref) => {
    /** States */
    ...
    const loadCardConfig = (cardNumber) => {
        ...
        const { type = '', code = { size: 0 }, gaps = [], lengths = [], niceType = '', } = card;
        setCardData((prev) => ({
            ...prev,
            brand: type,
        }));
    ...
    return (React.createElement(TouchableWithoutFeedback, { onPress: Keyboard.dismiss },
        React.createElement(View, { style: styles.cardWrapper },
            React.createElement(CardSide, { background: background, style: [[getSideStyle(CardSideEnum.FRONT)]] },
                React.createElement(View, { style: styles.header },
                    React.createElement(TouchableOpacity, { onPress: rotate },
                        React.createElement(Image, { source: Images.icons.rotate })),
                    !!cardConfig.brandImage && (React.createElement(Animatable.Image, { source: cardConfig.brandImage, animation: "slideInRight", duration: 400, useNativeDriver: true })),
                    !!(!cardConfig.brandImage && cardConfig.brandName) && (React.createElement(Animatable.Text, { style: [styles.textData, textStyle], animation: "slideInRight", duration: 400 }, cardConfig.brandName))),
                React.createElement(Image, { source: Images.chip, style: styles.imageChip }),
                React.createElement(View, null,
                    React.createElement(CardInput, { placeholderTextColor: placeholderTextColor, name: "number", onChange: handleInputChange, value: cardData?.number, autoFocus: true, placeholder: placeholders.number, style: [
                            styles.textCardNumber,
                            { color: errors.number ? errorTextColor : textColor },
                        ], maxLength: cardConfig.numberMask.length, autocompleteType: "cc-number", textContentType: "creditCardNumber", maskProps: {
                            type: 'custom',
                            options: { mask: cardConfig.numberMask },
                        }, refInput: numberInputRef, returnKeyType: "next", autoCorrect: false, onSubmitEditing: () => focusField(holderInputRef), blurOnSubmit: false })),
                React.createElement(View, { style: styles.footer },
                    React.createElement(View, { style: styles.holderWrapper },
                        React.createElement(Text, { style: [styles.textLabel, textStyle] }, labels.holder),
                        React.createElement(CardInput, { placeholderTextColor: placeholderTextColor, name: "holder", autocompleteType: "name", textContentType: "name", returnKeyType: "next", onChange: handleInputChange, placeholder: placeholders.holder, autoCapitalize: "characters", style: [
                                styles.textData,
                                { color: errors.holder ? errorTextColor : textColor },
                            ], value: cardData?.holder, refInput: holderInputRef, onSubmitEditing: () => focusField(expirationInputRef), blurOnSubmit: false })),
                    React.createElement(View, { style: styles.expirationWrapper },
                        React.createElement(Text, { style: [styles.textLabel, textStyle] }, labels.expiration),
                        React.createElement(CardInput, { placeholderTextColor: placeholderTextColor, name: "expiration", autocompleteType: "cc-exp", keyboardType: "numbers-and-punctuation", returnKeyType: "next", onChange: handleInputChange, placeholder: placeholders.expiration, style: [
                                styles.textData,
                                {
                                    color: errors.expiration &&
                                        cardData.expiration.length === expirationMask.length
                                        ? errorTextColor
                                        : textColor,
                                },
                            ], value: cardData?.expiration, maxLength: 7, maskProps: {
                                type: 'custom',
                                options: { mask: expirationMask },
                            }, refInput: expirationInputRef, onSubmitEditing: () => {
                                focusField(cvvInputRef);
                                rotate();
                            }, blurOnSubmit: false })))),
            React.createElement(CardSide, { style: [getSideStyle(CardSideEnum.BACK)], background: background },
                React.createElement(View, { style: styles.strip }),
                React.createElement(TouchableOpacity, { onPress: rotate },
                    React.createElement(Image, { source: Images.icons.rotate })),
                React.createElement(View, { style: styles.cvvWrapper, pointerEvents: "box-none" },
                    React.createElement(Text, { style: [styles.textLabel, textStyle] }, labels.cvv),
                    React.createElement(CardInput, { placeholderTextColor: placeholderTextColor, name: "cvv", autocompleteType: "cc-csc", keyboardType: "numeric", returnKeyType: "done", onChange: handleInputChange, placeholder: placeholders.cvv, style: [
                            styles.textData,
                            { color: errors.cvv ? errorTextColor : textColor },
                        ], value: cardData?.cvv, maxLength: cardConfig.cvvMask.length, maskProps: {
                            type: 'custom',
                            options: { mask: cardConfig.cvvMask },
                        }, onBlur: rotate, refInput: cvvInputRef }))))));
});
